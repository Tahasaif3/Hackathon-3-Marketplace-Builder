...
async function deleteAllDocuments(type) {
    try {
      ...
      console.log(`All ${type} documents deleted successfully`)
    } catch (error) {
...
async function importData() {
  try {
    console.log('Fetching food, chef data from API...');
      ...
      await deleteAllDocuments('food')
      await deleteAllDocuments('chef')
    ...
    $Promise.push(
      axios.get('http://localhost:3000/api/chefs')
    );
